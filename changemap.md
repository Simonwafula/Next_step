...
- [/] (T-1A0) Assignment-driven app hardening from PostgreSQL data-mining review
  ...
  - [x] (T-1A11) Add CSA 821 assignment notebook plus reproducible CSV/SQL snapshot for Kenya seniority classification
  - [x] (T-1A12) Add action-by-action production rollout TODO for seniority model improvements, testing, and push discipline
  ...
  - Log 2026-04-18: added `notebooks/csa821_assignment_1_nextstep_kenya_seniority_classification.ipynb`, `notebooks/data/csa821_kenya_seniority_dataset.csv`, and `notebooks/sql/csa821_kenya_seniority_dataset_export.sql` for a reproducible Kenya seniority-classification assignment snapshot and export workflow.
  - Log 2026-04-18: added `TODO_NEXTSTEP_SENIORITY_ROLLOUT.md` as the execution ledger for future seniority-model productionization work, including action sequencing, logging rules, test gates, push discipline, secret-handling constraints, and required decision logging when follow-up changes are discovered.
  - Log 2026-04-18: completed `T-1A12 Action 0` by freezing the current baseline in `docs/seniority_baseline.md` with the reproducible snapshot files, row count (`38,335`), class distribution (`Mid-Level 15,106`, `Senior 13,063`, `Entry 5,779`, `Executive 4,387`), top-source/title summary, and current sampled baseline metrics.
  - Log 2026-04-18: decided not to commit the full `notebooks/data/csa821_kenya_seniority_dataset.csv` snapshot because it is a large output (`~179 MB`) and would violate repo hygiene; the committed baseline keeps the notebook, SQL export, and baseline documentation, while the CSV remains a local regeneration artifact.
  - Log 2026-04-18: completed `T-1A12 Action 1` by adding `docs/seniority_rollout_policy.md` and choosing `internal_only` for model-predicted seniority; the policy allows bounded internal use in ranking/recommendation/alert ordering, defers soft user labels until reviewed-label and calibration gates exist, and explicitly prohibits hard filtering in the current rollout.
  - Log 2026-04-18: recorded the scope boundary that this policy governs future model-predicted seniority only; the existing ingestion-time `JobPost.seniority` field remains a separate signal and current search/filter behavior for that field is not changed by Action 1.
  - Log 2026-04-18: completed `T-1A12 Action 2` by adding `data/samples/seniority_review_sample.csv` as a repo-safe silver audit set with `20` reviewed rows, balanced `5` per current class, and deliberate lexical-trap cases like `sales executive` and `executive sous chef` to expose known label-noise patterns.
  - Log 2026-04-18: added `docs/seniority_labeling_guide.md` to define the review taxonomy, lexical traps, confidence rules, and repo-safe sample boundaries; reviewed data is now repo-safe at the anonymized title level, while raw-post back-links remain operational-only and out of git.
  ...
  - Tests 2026-04-18: `python3 -m json.tool notebooks/csa821_assignment_1_nextstep_kenya_seniority_classification.ipynb`; `/home/nextstep.co.ke/.venv/bin/python` smoke-run against PostgreSQL data to confirm the query and modelling pipeline execute on a sample; CSV-only smoke-run to confirm the same pipeline works without direct database access.
  - Tests 2026-04-18: manual review of `TODO_NEXTSTEP_SENIORITY_ROLLOUT.md` for action ordering, branch/commit conventions, log requirements, secret-handling rules, and per-action test gates.
  - Tests 2026-04-18: `python3 -m json.tool notebooks/csa821_assignment_1_nextstep_kenya_seniority_classification.ipynb` -> `NOTEBOOK_OK`; `/home/nextstep.co.ke/.venv/bin/python` CSV-only sampled smoke evaluation -> `logreg weighted_f1=0.5537`, `svm weighted_f1=0.6464`, `rows=38335`, `sampled_rows=3000`.
  - Tests 2026-04-18: manual policy review against the current shipped seniority touchpoints in `backend/app/api/routes.py`, `backend/app/services/search.py`, and `backend/app/services/ranking.py` to confirm the Action 1 rollout decision matches live search/ranking behavior and keeps model-predicted seniority out of hard filters.
  - Tests 2026-04-18: `backend/venv3.11/bin/ruff format backend/tests/test_seniority_review_sample.py` -> `1 file reformatted`; `backend/venv3.11/bin/ruff check backend/tests/test_seniority_review_sample.py` -> `All checks passed!`; `/home/nextstep.co.ke/.venv/bin/pytest -q backend/tests/test_seniority_review_sample.py` -> `2 passed`; `python3 -c '...csv.DictReader(...)'` -> `CSV_OK 20 True`.
