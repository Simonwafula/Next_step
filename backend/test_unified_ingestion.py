...
class UnifiedIngester:
    ...
    async def ingest_from_source(self, scraper_class, source_name, max_pages=2, max_jobs=10):
        ...
        for page in range(1, max_pages + 1):
            ...
            for job in listings[:max_jobs - jobs_processed]:
                ...
                job_id = await self.processor.process_job_url(job.full_link, source_name)
                if job_id:
                    jobs_processed += 1
                    page_success += 1
            ...
            pages_processed += 1
            logger.info(f"Page {page} complete: {page_success}/{len(listings)} jobs processed")
            
            if jobs_processed >= max_jobs:
                break
        
        scraper.db.close()
        logger.info(f"{source_name} ingestion complete: {jobs_processed} jobs from {pages_processed} pages")
    ...
    async def run_ingestion_test(self):
        ...
        if sources_working >= 2 and total_jobs >= 10:
            ...
            logger.error(f"Need ≥2 sources and ≥10 jobs, got {sources_working} sources and {total_jobs} jobs")
            return False
