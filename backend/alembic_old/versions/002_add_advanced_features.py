...
def upgrade() -> None:
    ...
    op.create_table('user_job_recommendations',
        ...
        sa.Column('missing_skills', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
        sa.Column('matching_skills', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
        sa.Column('recommended_at', sa.DateTime(), nullable=False, default=sa.func.now()),
        sa.Column('algorithm_version', sa.String(length=50), nullable=False, default='v1.0'),
    ...
    op.create_index('idx_user_job_recommendation', 'user_job_recommendations', ['user_id', 'job_post_id'], unique=True)
    op.create_index('ix_user_job_recommendations_recommended_at', 'user_job_recommendations', ['recommended_at'])
    ...
    op.create_index('idx_user_skill_assessment', 'skill_assessments', ['user_id', 'skill_id'])
    ...
    op.create_table('job_alerts',
        ...
        sa.Column('delivery_methods', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
    ...
    op.create_table('interview_preparations',
        ...
        sa.Column('questions_practiced', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
        sa.Column('research_notes', sa.Text(), nullable=True),
        sa.Column('preparation_score', sa.Float(), nullable=True),
        sa.Column('suggested_questions', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
        sa.Column('company_insights', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default={}),
        sa.Column('role_specific_tips', postgresql.JSONB(astext_type=sa.Text()), nullable=False, default=[]),
        sa.Column('created_at', sa.DateTime(), nullable=False, default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime(), nullable=False, default=sa.func.now()),
...
def downgrade() -> None:
    ...
    op.drop_index('ix_user_job_recommendations_recommended_at', table_name='user_job_recommendations')
