...
def upgrade():
    # Add url_hash column
    op.add_column('job_post', sa.Column('url_hash', sa.String(length=32), nullable=True))
    op.create_index(op.f('ix_job_post_url_hash'), 'job_post', ['url_hash'], unique=False)

    # Add repost_count column
    op.add_column('job_post', sa.Column('repost_count', sa.Integer(), nullable=False, server_default='0'))
