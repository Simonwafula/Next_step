...
async def test_extraction_improvements():
    ...
    async with JobDataExtractor() as extractor:
        for job in test_jobs:
            ...
            if data:
                ...
                filled_fields = sum(1 for field in fields if data.get(field, '').strip())
                ...
                print(f"   Job Type: {data.get('employment_type', 'NOT FOUND')[:30]}...")
    ...
    try:
        ...
        if total_jobs > 0:
            with_company = db.execute(text("SELECT COUNT(*) FROM job_post WHERE org_id IS NOT NULL")).scalar()
            with_location = db.execute(text("SELECT COUNT(*) FROM job_post WHERE location_id IS NOT NULL")).scalar()
            with_salary = db.execute(text("SELECT COUNT(*) FROM job_post WHERE salary_min IS NOT NULL")).scalar()
            
            print(f"Jobs with company: {with_company} ({with_company/total_jobs*100:.1f}%)")
            print(f"Jobs with location: {with_location} ({with_location/total_jobs*100:.1f}%)")
            print(f"Jobs with salary: {with_salary} ({with_salary/total_jobs*100:.1f}%)")
            
            overall_quality = ((with_company + with_location + with_salary) / (total_jobs * 3)) * 100
