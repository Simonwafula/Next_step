...
async def test_scraper_configurations():
    ...
    try:
        ...
        for site_name, config in configs.items():
            ...
            logger.info(
                f"  {site_name}: {status} ({response_time:.2f}s, {jobs_found} jobs found)"
            )
        ...
        logger.info(
            f"Scraper test summary: {successful}/{len(test_results)} successful ({success_rate:.1%})"
        )
...
async def test_workflow_stages():
    ...
    async for db in get_db():
        try:
            ...
            processing_results = await automated_workflow_service._run_processing_stage(
                db
            )
            logger.info(
                f"Processing stage: {processing_results.get('status', 'unknown')}"
            )
            ...
            optimization_results = (
                await automated_workflow_service._run_optimization_stage(db)
            )
            logger.info(
                f"Optimization stage: {optimization_results.get('status', 'unknown')}"
            )
...
async def test_complete_workflow():
    ...
    async for db in get_db():
        try:
            workflow_results = await automated_workflow_service.run_complete_workflow(
                db
            )

            logger.info(
                f"Complete workflow: {workflow_results.get('status', 'unknown')}"
            )
            logger.info(
                f"Duration: {workflow_results.get('duration_minutes', 0):.2f} minutes"
            )
...
def test_celery_tasks():
    ...
    try:
        ...
        processing_tasks = [
            task for task in registered_tasks if "processing_tasks" in task
        ]
