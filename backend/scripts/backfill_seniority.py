...
def extract_seniority(title: str, description: str) -> Optional[str]:
    ...
    seniority_patterns = {
        ...
        'management': [
            ...
            'team lead', 'team leader', 'lead ', ' lead,',
            'head of', 'head,', 'head -',
            'coordinator', 'principal officer',
            'director', 'assistant director', 'deputy director',
        ],
        'senior': [
            'senior ', 'sr.', 'sr ', 'snr ',
            'principal', 'staff ', 'lead developer', 'lead engineer',
        ...
        'mid': [
            ...
            'officer', 'analyst', 'accountant', 'engineer', 'developer',
        ],
    }
...
def is_real_job_title(title: str) -> bool:
    ...
    skip_patterns = [
        'vacanc', 'careers page', 'career opportunit', 'downloads', 'promotions',
        'order paper', 'hansard', 'circular', 'committee',
        'website', 'portal', 'login', 'form 1j', 'composition',
        'job description page', 'recruitment and selection',
        'advertised posts', 'faqs', 'active promotion', 'current advert',
        'track advert', 'reset grid', 'job posting', 'job openings',
        'loading', 'skip to content', 'shortlist', 'interview schedule',
        'tender', 'addendum', 'notice', 'constitution', 'published by',
        'county government of', 'county assembly', 'county public service',
        'investment opportunit', 'internship/attachment', 'membership',
        'read notice', 'download', 'view detail', 'e-recruitment',
        'job adverts', 'job advert archive', 'public appointments',
        'appointm', 'insurance advert', 'gorvenor', 'hon.', 'mca.',
        'tenders', 'clearance certificate', 'leave form', 'bio data',
        'spas form', 'hr manual', 'finance bill', 'application form',
    ]
    ...
    job_indicators = [
        'officer', 'manager', 'director', 'assistant', 'analyst',
        'engineer', 'developer', 'accountant', 'specialist', 'coordinator',
        'supervisor', 'technician', 'nurse', 'doctor', 'teacher',
        'clerk', 'secretary', 'administrator', 'executive', 'consultant',
        'driver', 'mechanic', 'plumber', 'electrician', 'technologist',
        'sales', 'marketing', 'finance', 'legal', 'medical',
        'chief officer', 'municipal manager', 'registrar', 'auditor',
    ]
...
def backfill_seniority(dry_run: bool = False):
    ...
    try:
        ...
        jobs_without_seniority = db.query(JobPost).filter(
        ...
        real_jobs = [j for j in jobs_without_seniority if is_real_job_title(j.title_raw)]
        print(f"Of those, {len(real_jobs)} have real job titles")

        updated_count = 0
        skipped_count = 0
        seniority_counts = {'entry': 0, 'mid': 0, 'senior': 0, 'management': 0, 'executive': 0}

        for job in real_jobs:
            seniority = extract_seniority(job.title_raw or '', job.description_raw or '')
            ...
            if seniority:
                ...
                if not dry_run:
                    ...
                    if updated_count < 10:  # Show first 10 examples
                        print(f"  Would update job {job.id}: {(job.title_raw or '')[:50]}... -> {seniority}")
        ...
        # Show final stats
        jobs_with_seniority = db.query(JobPost).filter(
            JobPost.seniority.isnot(None)
        ).count()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Backfill seniority data for existing jobs")
    parser.add_argument(
        "--dry-run",
        action="store_true",
