#!/usr/bin/env python3
"""
Run JobWebKenya ingestion into the main job database.

This script:
1. Scrapes jobs from JobWebKenya
...
def scrape_jobwebkenya_page(page_number: int) -> list:
    """Scrape a single page of JobWebKenya jobs."""
...
async def run_ingestion(pages: int = 5, batch_size: int = 10):
    """Run the JobWebKenya ingestion process."""
    logger.info(f"Starting JobWebKenya ingestion for {pages} pages")

    # Scrape all pages
    all_jobs = []
    for page in range(1, pages + 1):
        logger.info(f"Scraping page {page}/{pages}...")
        jobs = scrape_jobwebkenya_page(page)
