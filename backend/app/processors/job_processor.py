...
class JobProcessor:
    ...
    async def process_scraped_jobs(self, scraped_jobs: List[Dict]) -> Dict:
        ...
        job_urls = [
            {"url": job.get("link", ""), "source": job.get("source", "unknown")}
    ...
    async def reprocess_failed_jobs(self, max_retries: int = 3) -> Dict:
        ...
        return {"reprocessed": 0, "successful": 0, "still_failed": 0}
    ...
    async def update_existing_jobs(self, days_old: int = 7) -> Dict:
        ...
        try:
            ...
            old_jobs = db.query(JobPost).filter(JobPost.last_seen < cutoff_date).all()

            logger.info(f"Found {len(old_jobs)} jobs to update")

            # Convert to format for batch processing
            job_urls = [{"url": job.url, "source": job.source} for job in old_jobs]
...
class JobProcessorService:
    ...
    async def process_from_scraper_output(
        self, scraper_name: str, scraped_data: List[Dict]
    ) -> Dict:
        """
    ...
    async def process_single_url(
        self, url: str, source: str = "manual"
    ) -> Optional[int]:
        """
    ...
    async def health_check(self) -> Dict:
        ...
        try:
            ...
            async with JobDataExtractor():
                # This just tests that the extractor can be constructed and entered.
                pass
