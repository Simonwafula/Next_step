...
def extract_education_level(text: str) -> str | None:
    ...
    if any(m in t for m in ["master's", "masters", "msc", "mba", "ma degree"]):
        return "Master"
    if any(b in t for b in ["bachelor's", "bachelors", "degree", "university graduate", "bsc", "ba degree"]):
        return "Bachelor"
...
def classify_seniority(title: str, experience_years: int | None) -> str:
    ...
    if any(x in t for x in ["junior", "entry", "intern", "graduate", "trainee", "assistant"]):
        return "Entry"
