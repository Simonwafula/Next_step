...
class JobDataMigrator:
    ...
    def get_or_create_organization(self, org_name: str) -> Optional[int]:
        ...
        org = self.pg_session.query(Organization).filter(
    ...
    def migrate_jobs(self, batch_size: int = 1000) -> int:
        ...
        try:
            ...
            for offset in range(0, total_count, batch_size):
                ...
                logging.info(f"Processing batch {offset//batch_size + 1}: {len(batch)} jobs")
                
                for row in batch:
                    try:
                        # Check if job already exists
                        existing = self.pg_session.query(JobPost).filter(
                            JobPost.url == row['full_link']
                        ).first()
            ...
            logging.info(f"Migration completed: {migrated_count} migrated, {skipped_count} skipped")
