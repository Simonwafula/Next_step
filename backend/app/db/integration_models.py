...
class LinkedInProfile(Base):
    ...
    sync_frequency: Mapped[str] = mapped_column(String(50), default="weekly")  # daily, weekly, monthly
    last_synced: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    sync_status: Mapped[str] = mapped_column(String(50), default="active")  # active, error, disabled
...
class CalendarEvent(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    calendar_integration_id: Mapped[int] = mapped_column(ForeignKey("calendar_integrations.id"))
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    ...
    event_type: Mapped[str] = mapped_column(String(50))  # interview, deadline, reminder, meeting
    related_job_id: Mapped[int | None] = mapped_column(ForeignKey("job_post.id"), nullable=True)
    related_application_id: Mapped[int | None] = mapped_column(ForeignKey("job_applications.id"), nullable=True)
    
    # Meeting details (for interviews)
    meeting_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    meeting_platform: Mapped[str | None] = mapped_column(String(50), nullable=True)  # zoom, teams, meet
    ...
    reminder_times: Mapped[list] = mapped_column(JSONB, default=list)  # minutes before event
...
class ATSIntegration(Base):
    ...
    ats_provider: Mapped[str] = mapped_column(String(50))  # greenhouse, lever, workday, bamboohr, etc.
...
class ATSJobSync(Base):
    ...
    __table_args__ = (Index('idx_ats_job_sync', 'ats_integration_id', 'ats_job_id', unique=True),)
...
class ATSApplicationSync(Base):
    ...
    ats_submission_date: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    ...
    __table_args__ = (Index('idx_ats_app_sync', 'ats_integration_id', 'ats_application_id', unique=True),)
...
class IntegrationActivityLog(Base):
    ...
    organization_id: Mapped[int | None] = mapped_column(ForeignKey("organization.id"), nullable=True)
    
    # Integration details
    integration_type: Mapped[str] = mapped_column(String(50))  # linkedin, calendar, ats
    integration_id: Mapped[int] = mapped_column(Integer)  # ID of the specific integration
    
    # Activity details
    activity_type: Mapped[str] = mapped_column(String(100))  # sync, auth, error, webhook, etc.
    ...
    __table_args__ = (Index('idx_integration_activity', 'integration_type', 'integration_id', 'created_at'),)
