...
class User(Base):
    ...
    subscription_tier: Mapped[str] = mapped_column(String(50), default="basic")  # basic, professional, enterprise
...
class UserProfile(Base):
    ...
    experience_level: Mapped[str | None] = mapped_column(String(50), nullable=True)  # entry, mid, senior, executive
    education: Mapped[str | None] = mapped_column(Text, nullable=True)
    skills: Mapped[dict] = mapped_column(JSONB, default=dict)  # {"skill_name": confidence_score}
    ...
    salary_expectations: Mapped[dict] = mapped_column(JSONB, default=dict)  # {"min": 50000, "max": 80000, "currency": "KSH"}
...
class SavedJob(Base):
    ...
    folder: Mapped[str | None] = mapped_column(String(100), nullable=True)  # For organizing saved jobs
    
    # Relationships
    user: Mapped["User"] = relationship("User", back_populates="saved_jobs")
    job_post: Mapped["JobPost"] = relationship("JobPost")
    
    __table_args__ = (Index('idx_user_job_saved', 'user_id', 'job_post_id', unique=True),)
...
class JobApplication(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    job_post_id: Mapped[int] = mapped_column(ForeignKey("job_post.id"))
    status: Mapped[str] = mapped_column(String(50), default="applied")  # applied, screening, interview, offer, rejected, withdrawn
    ...
    application_source: Mapped[str | None] = mapped_column(String(100), nullable=True)  # direct, platform, referral
    ...
    __table_args__ = (Index('idx_user_job_application', 'user_id', 'job_post_id', unique=True),)
...
class SearchHistory(Base):
    ...
    filters: Mapped[dict] = mapped_column(JSONB, default=dict)  # location, seniority, etc.
    results_count: Mapped[int] = mapped_column(Integer, default=0)
    clicked_jobs: Mapped[list] = mapped_column(JSONB, default=list)  # job_post_ids that were clicked
...
class UserNotification(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    type: Mapped[str] = mapped_column(String(50))  # job_alert, career_advice, system, marketing
    title: Mapped[str] = mapped_column(String(255))
    message: Mapped[str] = mapped_column(Text)
    data: Mapped[dict] = mapped_column(JSONB, default=dict)  # Additional structured data
    
    # Delivery tracking
    is_read: Mapped[bool] = mapped_column(Boolean, default=False)
    delivered_via: Mapped[list] = mapped_column(JSONB, default=list)  # email, whatsapp, push, in_app
...
class UserJobRecommendation(Base):
    ...
    __table_args__ = (Index('idx_user_job_recommendation', 'user_id', 'job_post_id', unique=True),)
...
class CompanyReview(Base):
    ...
    user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)  # Anonymous reviews allowed
    ...
    employment_status: Mapped[str | None] = mapped_column(String(50), nullable=True)  # current, former
...
class SkillAssessment(Base):
    ...
    level: Mapped[str] = mapped_column(String(50))  # beginner, intermediate, advanced, expert
    percentile: Mapped[float | None] = mapped_column(Float, nullable=True)  # Compared to other users
    
    # Assessment metadata
    assessment_type: Mapped[str] = mapped_column(String(50))  # quiz, practical, portfolio
...
class JobAlert(Base):
    ...
    query: Mapped[str] = mapped_column(String(500))
    filters: Mapped[dict] = mapped_column(JSONB, default=dict)  # location, salary, seniority, etc.
    
    # Alert settings
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    frequency: Mapped[str] = mapped_column(String(50), default="daily")  # immediate, daily, weekly
    delivery_methods: Mapped[list] = mapped_column(JSONB, default=list)  # email, whatsapp, push
...
class InterviewPreparation(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    job_application_id: Mapped[int | None] = mapped_column(ForeignKey("job_applications.id"), nullable=True)
    
    # Interview details
    company_name: Mapped[str] = mapped_column(String(255))
    role_title: Mapped[str] = mapped_column(String(255))
    interview_type: Mapped[str] = mapped_column(String(50))  # phone, video, in_person, technical
    ...
    preparation_score: Mapped[float | None] = mapped_column(Float, nullable=True)  # 0.0 to 100.0
...
class UserAnalytics(Base):
    ...
    event_type: Mapped[str] = mapped_column(String(50))  # search, view_job, save_job, apply, etc.
