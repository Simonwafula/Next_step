...
async def sync_linkedin_profile(
    ...
    try:
        ...
        if (
            linkedin_profile.token_expires_at
            and linkedin_profile.token_expires_at <= datetime.utcnow()
        ):
...
async def calendar_auth_url(
    ...
    try:
        ...
        return CalendarAuthResponse(
            authorization_url=authorization_url, provider=provider, state=state
        )
...
async def calendar_callback(
    ...
    try:
        ...
        # Exchange code for token
        if provider == "google":
            token_data = await calendar_service.exchange_google_code_for_token(
                code, state
            )
            user_data = await calendar_service.get_google_user_info(
                token_data["access_token"]
            )
        elif provider == "microsoft":
            token_data = await calendar_service.exchange_microsoft_code_for_token(code)
            user_data = await calendar_service.get_microsoft_user_info(
                token_data["access_token"]
            )
        else:
            raise HTTPException(
...
async def get_integration_activity(
    ...
    try:
        ...
        if integration_type:
            query = query.where(
                IntegrationActivityLog.integration_type == integration_type
            )
