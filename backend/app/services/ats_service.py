...
from ..db.integration_models import ATSIntegration, ATSJobSync, ATSApplicationSync, IntegrationActivityLog
import logging
...
class ATSService:
    def __init__(self):
        self.supported_ats = {
            'greenhouse': {
                ...
                'auth_type': 'basic',
                'webhook_events': ['job.created', 'job.updated', 'application.created', 'application.updated']
            },
            'lever': {
                'base_url': 'https://api.lever.co/v1/',
                'auth_type': 'basic',
                'webhook_events': ['posting.created', 'posting.updated', 'application.created', 'application.updated']
            },
            'workday': {
                'base_url': 'https://wd2-impl-services1.workday.com/',
                'auth_type': 'oauth',
                'webhook_events': ['job.created', 'job.updated', 'candidate.created', 'candidate.updated']
            },
            'bamboohr': {
    ...
    async def create_ats_integration(
        ...
        try:
            ...
            if not connection_test['success']:
                raise Exception(f"ATS connection test failed: {connection_test['error']}")
            ...
            if settings_data:
                ...
                ats_integration.sync_applications = settings_data.get('sync_applications', True)
                ats_integration.sync_candidates = settings_data.get('sync_candidates', False)
                ats_integration.webhook_events = settings_data.get('webhook_events', 
                    self.supported_ats[ats_provider]['webhook_events'])
            ...
            await self._log_activity(
                db, None, organization_id, ats_integration.id,
                "integration_created", f"{ats_provider.title()} ATS integration created",
    ...
    async def _test_ats_connection(self, ats_provider: str, credentials: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _test_greenhouse_connection(self, credentials: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    'https://harvest-api.greenhouse.io/v1/users',
                    headers=headers
                )
    ...
    async def _test_lever_connection(self, credentials: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _test_workday_connection(self, credentials: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not access_token or not instance_url:
                return {'success': False, 'error': 'Access token and instance URL are required'}
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{instance_url}/ccx/api/v1/workers",
                    headers=headers
                )
    ...
    async def _test_bamboohr_connection(self, credentials: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not api_key or not company_id:
                return {'success': False, 'error': 'API key and company ID are required'}
    ...
    async def sync_jobs_from_ats(self, db: AsyncSession, ats_integration: ATSIntegration) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log activity
            await self._log_activity(
                db, None, ats_integration.organization_id, ats_integration.id,
                "jobs_synced", f"Synced {jobs_processed} jobs from {ats_integration.ats_provider}",
                {"jobs_processed": jobs_processed, "jobs_created": jobs_created, "jobs_updated": jobs_updated}
            )
    ...
    async def _get_greenhouse_jobs(self, ats_integration: ATSIntegration) -> Dict[str, Any]:
        """Get jobs from Greenhouse API"""
        try:
            auth_string = base64.b64encode(f"{ats_integration.api_key}:".encode()).decode()
            headers = {
                'Authorization': f'Basic {auth_string}',
                'Content-Type': 'application/json'
            }
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    'https://harvest-api.greenhouse.io/v1/jobs',
                    headers=headers
                )
                ...
                if response.status_code == 200:
                    ...
                    return {'success': False, 'error': f'Failed to fetch jobs: {response.text}'}
    ...
    async def _get_workday_jobs(self, ats_integration: ATSIntegration) -> Dict[str, Any]:
    ...
    async def _get_bamboohr_jobs(self, ats_integration: ATSIntegration) -> Dict[str, Any]:
        """Get jobs from BambooHR API"""
        try:
            auth_string = base64.b64encode(f"{ats_integration.api_key}:x".encode()).decode()
            headers = {
                'Authorization': f'Basic {auth_string}',
                'Content-Type': 'application/json'
            }
    ...
    def _extract_workday_job_info(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'status': 'open' if job_data.get('jobPostingStatus') == 'Active' else 'closed',
    ...
    def _create_job_post_from_ats(self, ats_integration: ATSIntegration, job_info: Dict[str, Any]) -> JobPost:
    ...
    def _update_job_post_from_ats(self, job_post: JobPost, job_info: Dict[str, Any]):
        ...
        job_post.requirements_raw = job_info.get('requirements', job_post.requirements_raw)
    ...
    async def submit_application_to_ats(
        ...
        try:
            ...
            if ats_integration.ats_provider == 'greenhouse':
                ...
                return {'success': False, 'error': 'Application submission not supported for this ATS'}
            ...
            if result['success']:
                ...
                job_application.application_source = f"ats_{ats_integration.ats_provider}"
                
                await db.commit()
                
                # Log activity
                await self._log_activity(
                    db, user.id, ats_integration.organization_id, ats_integration.id,
                    "application_submitted", "Application submitted to ATS",
                    {"ats_application_id": result['ats_application_id']}
                )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 201:
                    ...
                    return {'success': False, 'error': f'Greenhouse submission failed: {response.text}'}
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    return {'success': False, 'error': f'Lever submission failed: {response.text}'}
    ...
    async def _setup_greenhouse_webhook(self, ats_integration: ATSIntegration, webhook_url: str):
    ...
    async def _setup_lever_webhook(self, ats_integration: ATSIntegration, webhook_url: str):
        ...
        try:
            ...
            for integration in integrations:
                try:
                    ...
                    logger.error(f"Error syncing ATS integration {integration.id}: {str(e)}")
