...
class PaymentService:
    ...
    async def initiate_mpesa_payment(
        self, user_id: int, plan_id: str, phone_number: str
    ) -> Dict:
        """
        ...
        try:
            ...
            if event["type"] == "payment_intent.succeeded":
                ...
                await self._update_payment_status(
                    ...
                    status="failed",
                    payment_details={
                        "failure_reason": payment_intent.get(
                            "last_payment_error", {}
                        ).get("message")
                    },
                )
    ...
    async def _update_payment_status(
        self, external_id: str, status: str, payment_details: Dict = None
    ):
    ...
    async def _activate_subscription(self, external_id: str):
        ...
        try:
            payment = (
                db.query(Payment)
                .filter(
                    Payment.external_id == external_id, Payment.status == "completed"
                )
                .first()
            )
            ...
            if not payment:
                logger.error(
                    f"Completed payment with external_id {external_id} not found"
                )
                return
        ...
        try:
            ...
            if subscription:
                return {
                    ...
                    "plan_name": self.payment_plans.get(subscription.plan_id, {}).get(
                        "name"
                    ),
            ...
            return [
                {
                    "id": payment.id,
                    "plan_id": payment.plan_id,
                    "plan_name": self.payment_plans.get(payment.plan_id, {}).get(
                        "name"
                    ),
