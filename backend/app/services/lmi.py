...
def get_weekly_insights(db: Session, location: str | None = None) -> dict:
    ...
    # Current week data
    current_week_stmt = (
        select(JobPost, Organization, TitleNorm)
    ...
    return {
        ...
        "median_salary": calculate_median(current_salaries)
        if current_salaries
        else None,
        "salary_coverage": f"{len(current_salaries)}/{len(current_week_jobs)} postings ({len(current_salaries) / len(current_week_jobs) * 100:.0f}%)"
        if current_week_jobs
        else "0%",
        "tenure_mix": [
            {
                "type": tenure,
                "count": count,
                "percentage": round(count / len(current_week_jobs) * 100, 1),
            }
...
def get_market_trends(db: Session, days: int = 30, location: str | None = None) -> dict:
    ...
    daily_counts_stmt = (
        ...
        .join(Location, Location.id == JobPost.location_id, isouter=True)
        .where(and_(*conditions))
        .group_by(func.date(JobPost.first_seen))
        .order_by("date")
    )
    ...
    role_trends_stmt = (
        ...
        .join(Location, Location.id == JobPost.location_id, isouter=True)
        .where(and_(*conditions))
        .group_by(TitleNorm.family, func.date(JobPost.first_seen))
        .order_by("date")
    )
    ...
    return {
        ...
        "daily_posting_counts": [
            {"date": str(date), "postings": count} for date, count in daily_counts
        ],
        "average_daily_postings": sum(count for _, count in daily_counts)
        / len(daily_counts)
        if daily_counts
        else 0,
...
def get_salary_insights(
    db: Session, role_family: str | None = None, location: str | None = None
) -> dict:
    ...
    role_salary_stmt = (
        select(
            ...
            .within_group(JobPost.salary_min)
            .label("median_salary"),
            func.count(JobPost.id).label("count"),
        )
...
def get_trending_skills(db: Session, days: int = 7) -> list[dict]:
    ...
    # Current skills
    current_skills_stmt = (
        select(Skill.name, func.count(JobSkill.id).label("current_count"))
        .join(JobSkill, JobSkill.skill_id == Skill.id)
        .join(JobPost, JobPost.id == JobSkill.job_post_id)
        .where(JobPost.first_seen >= current_period)
        .group_by(Skill.name)
    )

    current_skills = {
        name: count for name, count in db.execute(current_skills_stmt).all()
    }

    # Previous period skills
    prev_skills_stmt = (
        select(Skill.name, func.count(JobSkill.id).label("prev_count"))
        .join(JobSkill, JobSkill.skill_id == Skill.id)
        .join(JobPost, JobPost.id == JobSkill.job_post_id)
        .where(JobPost.first_seen >= prev_period, JobPost.first_seen < current_period)
        .group_by(Skill.name)
    )
    ...
    for skill, current_count in current_skills.items():
        ...
        if (
            growth_rate > 20 and current_count >= 3
        ):  # Significant growth with minimum volume
            trending.append(
                {
                    "skill": skill,
...
def calculate_growth_rates(daily_counts: list) -> dict:
    ...
    prev_week = (
        sum(count for _, count in daily_counts[-14:-7])
    ...
    return {"weekly_growth": round(weekly_growth, 1), "trend": trend}
...
def assess_market_temperature(daily_counts: list) -> str:
    ...
    recent_avg = (
        sum(count for _, count in daily_counts[-7:]) / 7
        if len(daily_counts) >= 7
        else 0
    )
