...
class NotificationService:
    ...
    async def process_new_opportunities(self) -> Dict:
        ...
        try:
            ...
            new_opportunities = await data_processing_service.process_new_opportunities()
    ...
    def _job_matches_preferences(self, job: Dict, prefs: NotificationPreference) -> bool:
        """Check if a job matches user notification preferences"""
        # Location matching
        if prefs.preferred_locations:
            locations = [loc.strip().lower() for loc in prefs.preferred_locations.split(',')]
        ...
        if prefs.preferred_seniority:
            seniority_levels = [level.strip().lower() for level in prefs.preferred_seniority.split(',')]
...
"""
    ...
    async def _send_email_notifications(self, user: User, jobs: List[Dict]):
        ...
        logger.info(f"Would send email notification to {user.email} for {len(jobs)} jobs")
    ...
    async def send_custom_notification(self, user_id: int, message: str, notification_type: str = "custom") -> bool:
    ...
    async def get_notification_stats(self) -> Dict:
        ...
        try:
            ...
            total_sent = db.query(func.sum(NotificationLog.notifications_sent)).filter(
                NotificationLog.processed_at > yesterday
            ).scalar() or 0
            
            unique_users = db.query(func.count(NotificationLog.user_id.distinct())).filter(
                NotificationLog.processed_at > yesterday,
                NotificationLog.user_id.is_not(None)
            ).scalar() or 0
            
            total_users_with_prefs = db.query(func.count(User.id)).join(NotificationPreference).filter(
                NotificationPreference.enabled == True
            ).scalar() or 0
