...
class PersonalizedRecommendationService:
    ...
    async def generate_recommendations_for_user(
        ...
        try:
            ...
            stmt = (
                ...
                .where(JobPost.first_seen >= cutoff_date)
            )
            ...
            if profile.experience_level:
                exp_conditions = self._get_experience_conditions(
            ...
            if profile.salary_expectations:
                salary_conditions = self._get_salary_conditions(
            ...
            if user_keywords and not (
                ...
                or profile.experience_level
                or profile.salary_expectations
            ):
                keyword_conditions = []
            ...
            for job_post, org, location in results:
                try:
                    ...
                    final_score = match_data["overall_score"] * (
                        0.75 + 0.25 * recency_score
                    )
                    ...
                    recommendation = {
                        ...
                        "description_preview": job_post.description_raw[:200] + "..."
    ...
    async def get_stored_recommendations(
        ...
        try:
            ...
            stmt = (
                select(UserJobRecommendation, JobPost, Organization, Location)
                .join(JobPost, JobPost.id == UserJobRecommendation.job_post_id)
                .join(Organization, Organization.id == JobPost.org_id, isouter=True)
                .join(Location, Location.id == JobPost.location_id, isouter=True)
                .where(
                    and_(
            ...
            logger.error(
                f"Error getting stored recommendations for user {user_id}: {e}"
            )
            return []

    async def mark_recommendation_interaction(
        self, db: Session, user_id: int, job_id: int, interaction_type: str
    ) -> bool:
    ...
    async def get_recommendation_insights(
        ...
        try:
            ...
            top_skills_stmt = (
                select(UserJobRecommendation.matching_skills)
                .where(
                    and_(
                        UserJobRecommendation.user_id == user_id,
            ...
            return {
                ...
                "view_rate": round(viewed_count / total_recommendations * 100, 1)
                if total_recommendations > 0
                else 0,
                "click_rate": round(clicked_count / total_recommendations * 100, 1)
                if total_recommendations > 0
                else 0,
                ...
                "recommendation_quality": self._assess_recommendation_quality(
                    avg_match_score, viewed_count, clicked_count, total_recommendations
                ),
            }

        except Exception as e:
            logger.error(
                f"Error getting recommendation insights for user {user_id}: {e}"
            )
            return {}
    ...
    def _get_applied_job_ids(self, db: Session, user_id: int) -> List[int]:
        ...
        try:
            stmt = select(JobApplication.job_post_id).where(
    ...
    def _get_user_keywords(self, db: Session, user: User) -> List[str]:
        ...
        for field in (
            (profile.current_role, profile.career_goals, profile.education)
            if profile
            else ()
        ):
            if not field:
    ...
    def _assess_recommendation_quality(
        self, avg_score: float, viewed: int, clicked: int, total: int
    ) -> str:
