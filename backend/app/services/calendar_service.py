...
from ..db.integration_models import CalendarIntegration, CalendarEvent, IntegrationActivityLog
import logging
...
class CalendarService:
    def __init__(self):
        ...
        self.redirect_uri = f"{settings.API_BASE_URL}/api/v1/integrations/calendar/callback"
    ...
    async def exchange_google_code_for_token(self, code: str, state: str = None) -> Dict[str, Any]:
    ...
    async def create_or_update_calendar_integration(
        ...
        try:
            ...
            await self._log_activity(
                db, user_id, calendar_integration.id,
                "integration_created", f"{provider.title()} calendar integration created",
    ...
    async def create_interview_event(
        ...
        try:
            ...
            if not calendar_integration:
                logger.warning(f"No active calendar integration found for user {user_id}")
                return None
            ...
            if calendar_integration.token_expires_at <= datetime.utcnow():
                if not await self.refresh_access_token(db, calendar_integration):
                    logger.error(f"Failed to refresh token for calendar integration {calendar_integration.id}")
                    return None
            ...
            calendar_event = CalendarEvent(
                ...
                reminder_times=event_details.get('reminder_times', [15, 60])  # 15 min and 1 hour before
            )
            ...
            await self._log_activity(
                db, user_id, calendar_integration.id,
                "event_created", "Interview event created in calendar",
    ...
    async def _create_google_event(
        ...
        try:
            ...
            event_data = {
                ...
                'attendees': [
                    {'email': attendee} for attendee in event_details.get('attendees', [])
                ],
                'reminders': {
                    'useDefault': False,
                    'overrides': [
    ...
    async def _create_microsoft_event(
        ...
        try:
            ...
            event_data = {
                ...
                'reminderMinutesBeforeStart': min(event_details.get('reminder_times', [15]))
            }
            
            # Add meeting URL if provided
            if event_details.get('meeting_url'):
                event_data['body']['content'] += f"<br><br>Meeting Link: <a href='{event_details.get('meeting_url')}'>{event_details.get('meeting_url')}</a>"
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    expires_in = token_data.get('expires_in', 3600)
                    calendar_integration.token_expires_at = datetime.utcnow() + timedelta(seconds=expires_in)
                    calendar_integration.updated_at = datetime.utcnow()
            ...
            for event in events:
                try:
                    ...
                    for reminder_minutes in event.reminder_times:
                        if abs(minutes_until - reminder_minutes) <= 5:  # 5-minute tolerance
    ...
    async def _send_interview_reminder(self, db: AsyncSession, event: CalendarEvent):
        ...
        try:
            ...
            # Log activity
            await self._log_activity(
                db, user.id, event.calendar_integration_id,
                "reminder_sent", "Interview reminder sent",
