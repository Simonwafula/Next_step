...
def get_skills_for_role(
    db: Session, role_title: str, limit: int = 20
) -> list[tuple[str, float]]:
    """Get most common skills for a specific role with frequency scores"""

    # Find jobs matching this role
    stmt = (
        select(JobPost, Skill, JobSkill.confidence)
        .join(JobSkill, JobPost.id == JobSkill.job_post_id)
        .join(Skill, Skill.id == JobSkill.skill_id)
        .where(JobPost.title_raw.ilike(f"%{role_title}%"))
    )
...
def calculate_skill_overlap(
    current_skills: list[str], target_skills: list[tuple[str, float]]
    ...
    overlap_percentage = (
        (len(overlapping) / len(target_skills_names)) * 100
        if target_skills_names
        else 0
    )
    ...
    for skill_name, frequency in target_skills[:10]:  # Focus on top 10 skills
        if (
            skill_name.lower() not in current_skills_lower and frequency > 20
        ):  # Only high-frequency skills
...
def transitions_for(db: Session, current: str):
    ...
    for title_norm in titles:
        ...
        target_embedding = embed_text(
            f"{title_norm.family} {title_norm.canonical_title}"
        )
        ...
        skill_overlap, missing_skills = calculate_skill_overlap(
    ...
    realistic_transitions = [
        t for t in scored_transitions if t["overlap"] > 30 or t["market_demand"] > 5
    ]
...
def get_trending_transitions(db: Session, days: int = 30) -> list[dict]:
    ...
    stmt = (
        select(JobPost.title_raw, func.count(JobPost.id).label("count"))
        .where(JobPost.first_seen >= cutoff_date)
        .group_by(JobPost.title_raw)
        .having(func.count(JobPost.id) > 2)
        .order_by(desc("count"))
    )
...
def get_salary_insights_for_transition(db: Session, target_role: str) -> dict:
    ...
    stmt = (
        select(JobPost.salary_min, JobPost.salary_max)
        .where(
            JobPost.title_raw.ilike(f"%{target_role}%"), JobPost.salary_min.is_not(None)
        )
