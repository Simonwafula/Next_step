...
class AutomatedWorkflowService:
    ...
    async def run_complete_workflow(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            results["duration_minutes"] = (
                results["completed_at"] - workflow_start
            ).total_seconds() / 60
        ...
        try:
            ...
            for site_name, config in scraper_configs.items():
                ...
                if test_result["status"] == "success":
                    scraping_result = await scraper_service.run_scraper_for_site(
                        site_name
                    )
                    stage_results["scraping_results"][site_name] = scraping_result
                    stage_results["total_jobs_scraped"] += scraping_result.get(
                        "jobs_scraped", 0
                    )
        ...
        try:
            ...
            for job_data in unprocessed_jobs:
                try:
                    ...
                    quality_score = await self._calculate_job_quality_score(
                        processed_job
                    )
                    ...
                    if quality_score >= self.quality_threshold:
                        ...
                        skills_count = await self._extract_and_save_skills(
                            db, processed_job
                        )
            ...
            stage_results["organizations_created"] = await self._update_organizations(
                db
            )

            stage_results["average_quality_score"] = (
                np.mean(stage_results["quality_scores"])
                if stage_results["quality_scores"]
                else 0
            )
            stage_results["completed_at"] = datetime.utcnow()
            stage_results["status"] = "success"
    ...
    async def _run_optimization_stage(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            stage_results["recommendation_model_updated"] = recommendation_update[
                "success"
            ]
    ...
    async def _get_scraper_configs(self) -> Dict[str, Any]:
        ...
        try:
            config_path = (
                Path(__file__).resolve().parents[1] / "scrapers" / "config.yaml"
            )
    ...
    async def _get_unprocessed_jobs(self, db: AsyncSession) -> List[Dict]:
        ...
        result = await db.execute(
            select(JobPost)
            .where(JobPost.first_seen >= datetime.utcnow() - timedelta(days=1))
            .limit(1000)
        )
            ...
            for pattern in title_patterns:
                if pattern["confidence"] > 0.8:
                    update_title_mappings(
                        pattern["raw_title"], pattern["normalized_title"]
                    )
    ...
    async def _learn_skills(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await db.execute(
                select(JobPost.description_raw, JobPost.requirements_raw)
                .where(JobPost.first_seen >= datetime.utcnow() - timedelta(days=7))
                .limit(500)
            )

            job_texts = [
                f"{desc} {req}" for desc, req in result.fetchall() if desc or req
            ]
            ...
            for skill_pattern in skill_patterns:
                if skill_pattern["confidence"] > 0.8:
                    update_skill_mappings(
                        skill_pattern["skill_name"], skill_pattern["normalized_skill"]
                    )
    ...
    async def _update_embeddings_model(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(recent_jobs) >= self.learning_threshold:
                ...
                job_texts = [
                    f"{job.title_raw} {job.description_raw}"
                ...
                update_results["model_performance"] = model_update.get(
                    "performance", {}
                )
    ...
    async def _generate_daily_metrics(self, db: AsyncSession, date) -> int:
        ...
        try:
            ...
            result = await db.execute(
                select(JobPost).where(func.date(JobPost.first_seen) == date)
            )
    ...
    async def _generate_skill_trends(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            current_skills = await self._get_skills_for_period(
                db, current_week, datetime.utcnow()
            )
            previous_skills = await self._get_skills_for_period(
                db, previous_week, current_week
            )
    ...
    async def _log_workflow_completion(self, db: AsyncSession, results: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Workflow {results['workflow_id']} completed in {results.get('duration_minutes', 0):.2f} minutes"
            )
            ...
            if results["status"] == "success":
                for stage_name, stage_results in results["stages"].items():
                    logger.info(
                        f"Stage {stage_name}: {stage_results.get('status', 'unknown')}"
                    )
    ...
    async def _update_user_preferences(
        self, db: AsyncSession, user_id: int, preferences: Dict
    ):
