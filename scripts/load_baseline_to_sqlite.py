...
from app.db.models import JobPost, JobEntities, JobEmbedding, TitleNorm
...
def load_baseline_to_sqlite(artifact_dir):
    ...
    try:
        ...
        # 2. Dummy TitleNorm
        tn = TitleNorm(
            id=1, family="software_development", canonical_title="Software Engineer"
        )
        db.add(tn)
        db.commit()
        ...
        for _, row in emb_df.iterrows():
            ...
            emb = JobEmbedding(
                job_id=int(row["job_id"]), model_name=row["model_name"], vector_json=vec
            )
            db.add(emb)
        db.commit()
