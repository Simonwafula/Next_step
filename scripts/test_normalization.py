...
def test_normalization_on_real_data(db_path, limit=50):
    ...
    print(f"{'Original Title':<40} | {'Normalized Title':<30} | {'Location':<15} | {'Company':<15}")
    print("-" * 110)
    ...
    for title, content in rows:
        ...
        # Skills
        skills = extract_and_normalize_skills(content)
        
        print(f"{title[:38]:<40} | {canon_title[:28]:<30} | {norm_loc[0]:<15} | {norm_company:<15}")
