...
def run_pipeline(db_path, output_dir, limit=1000):
    ...
    df_raw = pd.read_sql_query(
        "SELECT id, title, content, full_link as url FROM jobs_data LIMIT ?",
        conn,
        params=(limit,),
    )
    conn.close()
    ...
    for _, row in tqdm(df_raw.iterrows(), total=len(df_raw)):
        ...
        company_raw = (
            row["title"].split(" at ")[-1] if " at " in row["title"] else "Unknown"
        )
    ...
    # Save artifacts
    pd.DataFrame(normalized_jobs).to_csv(
        os.path.join(output_dir, "jobs_normalized.csv"), index=False
    )
