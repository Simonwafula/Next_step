...
if not DB_PATH:
    for p in [
        REPO / "jobs.sqlite3",
        REPO / "backend" / "var" / "nextstep.sqlite",
        REPO / "backend" / "jobs.sqlite3",
    ]:
        if p.exists():
            DB_PATH = str(p)
            break
...
def main():
    ...
    rc, out = sh(
        [
            "bash",
            "-lc",
            f"command -v tree >/dev/null && tree -L 4 -I '.git|venv*|.venv*|node_modules' || {tree_cmd}",
        ]
    )
    ...
    pytest_path = (
        str(venv_bin / "pytest") if (venv_bin / "pytest").exists() else "pytest"
    )
    ...
    for name, cmd in [
        (
            ...
            [
                "bash",
                "-lc",
                f"{ruff_path} --version && {ruff_path} check . && {ruff_path} format --check .",
            ],
        ),
        ("Pytest", ["bash", "-lc", f"{pytest_path} -q"]),
        (
            "Mypy (optional)",
            ["bash", "-lc", "mypy --version && mypy src || echo 'mypy not configured'"],
        ),
    ]:
        rc, out = sh(cmd)
        parts.append(md_block(name, f"(rc={rc})\n{out}"))
