...
def test_job_data_quality():
    ...
    try:
        with engine.connect() as conn:
            ...
            if title_quality and company_quality and location_quality and description_quality:
                logger.info("✅ Data quality criteria PASSED")
                return True
            else:
                logger.warning("⚠️ Data quality criteria FAILED - need structured extraction")
                return False
...
def test_ingestion_pipeline():
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("SELECT COUNT(*) FROM job_post WHERE id >= 1289"))
            recent_jobs = result.scalar()
            
            if recent_jobs >= 10:
                logger.info(f"✅ Ingestion pipeline working: {recent_jobs} jobs processed")
                return True
            else:
                logger.error(f"❌ Ingestion pipeline failed: only {recent_jobs} jobs processed")
                return False
